...
def testR4():
  ...
  if numch > 2:
    time.sleep(1)
    ret = L.usb_relay_device_open_one_relay_channel(hdev,3)
    if ret != 0: failed("Failed R3 on!")
    mask |= 0x4

  if numch > 3:
    time.sleep(1)
    ret = L.usb_relay_device_open_one_relay_channel(hdev,4)
    if ret != 0: failed("Failed R4 on!")
    mask |= 0x8
